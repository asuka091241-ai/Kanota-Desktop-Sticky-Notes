...
![Kanota 主界面](screenshots/app.png)
...
> 免安装，数据自动保存在解压目录的 `data/` 文件夹下。发给同学直接打包 `data/` 一起就行。
...
├── index.html         # 看板主界面（三列看板 + 侧边栏）
├── sticky.html        # 桌面便签界面（折叠/拖拽/番茄钟/专注模式）
